...
public class DriveForwardEncoders extends CommandBase {
  ...
  {
    return (((Math.abs(m_drive.m_encoder_left_1.getPosition()) + Math.abs(m_drive.m_encoder_left_2.getPosition()) + Math.abs(m_drive.m_encoder_right_1.getPosition()) + Math.abs(m_drive.m_encoder_right_2.getPosition())) /4.0) >= m_distance);
  }
}
